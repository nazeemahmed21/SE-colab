import "../styles/fms.css";
import "font-awesome/css/font-awesome.min.css"; // Import Font Awesome if you plan to use its icons
import '../ffolders.min.css'; // Import the FFolders.css library
import "../css-file-icons.css"; // Import the css-file-icons.css file
...
function FileSystem() {
  ...
  const fetchFiles = async () => {
    try {
      ...
      if (currentFolder) {
        const folderPath = `folders/${currentFolder.name}`;
  ...
  const navigateIntoFolder = async (folder) => {
    try {
      const folderPath = `folders/${folder.name}`;
      const folderRef = ref(storage, folderPath);
